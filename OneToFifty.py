...
class OneToFifty(Topo):
    def build(self):
        ...
        self.addLink(server, switch, cls=TCLink, bw=1000, delay='0.1ms', loss=0.1)
        
        for i in range(2, 52):
            host = self.addHost('h{}'.format(i), cls=Host, defaultRoute=None)
            self.addLink(host, switch, cls=TCLink, bw=20, delay='0.1ms', loss=0.1)
...
def main():
    ...
    for i in range(2, 52):  # Hosts from h2 to h51
        ...
        start_iperf_client(client, server_ip, 60, log_file)
