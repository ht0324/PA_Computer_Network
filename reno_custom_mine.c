...
static u32 global_rtt = 0;  // Global variable for storing the latest RTT
...
void tcp_reno_init(struct sock *sk)
{
    ...
    tcp_sk(sk)->snd_cwnd = 2; // Start with a congestion window of 2
}
...
u32 tcp_reno_ssthresh(struct sock *sk)
{
    ...
    u32 new_ssthresh = max((tp->snd_cwnd * 7) >> 3U, 2U); // Multiply by 3 and then right shift by 2 to divide by 4
    return new_ssthresh;
}

void tcp_cong_avoid_custom(struct tcp_sock *tp, u32 w, u32 acked)
{
    int multiplier = 1;
    /* If credits accumulated at a higher w, apply them gently now. */
    if (tp->snd_cwnd_cnt >= w) {
        tp->snd_cwnd_cnt = 0;
        tp->snd_cwnd += multiplier;
    }
...
void tcp_reno_cong_avoid(struct sock *sk, u32 ack, u32 acked)
{
    ...
    if (tp->snd_cwnd <= tp->snd_ssthresh) {
        ...
        tcp_cong_avoid_custom(tp, tp->snd_cwnd, acked);
    }
